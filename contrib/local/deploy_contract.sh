...
if [ -z $code_id ]
then 
    store_ret=$(wasmd tx wasm store $contract_path --from $ADMIN --gas="auto" --gas-adjustment 1.2 --chain-id=$CHAIN_ID --keyring-backend test -y)
    echo $store_ret
...
admin=$(wasmd keys show $ADMIN --keyring-backend test --output json | jq -r '.address')

wasmd tx wasm instantiate $code_id "$init" --from $ADMIN --label "$label" --gas auto --keyring-backend test --gas-adjustment 1.2 --admin $admin --chain-id=$CHAIN_ID -y
